...
class Route(object):
    def __init__(self, route=None, cost=0, is_valid=False, demand=0):
        if route is None:
            route = []
    ...
    def cast_list_int(self):
        list_cast = []
        for aux in self.route:
            list_cast.append(int(aux))
        return list_cast

    def __repr__(self):
        debug_str = ", cost = " + str(self.cost) + ", demand = " + str(self.demand)
        ret_str = (",".join(str(n) for n in self.route))
        listInt = [int(x) for x in ret_str.split(",")]
        return str(listInt) + (debug_str if False else "")
...
class CVRPInfo(object):
    ...
    def read_data(self, data_file):
        ...
        assert os.path.exists(data_file), data_file + ' - arquivo não existe.'
        ...
        index = 1
        for i in list(problem.node_coords.values()):
            xc, yc = [float(x) for x in i]
            self.coords[int(index)] = (xc, yc)
            index = index + 1
        indexB = 1
        for i in list(problem.demands.values()):
            dem = int(i)
            self.demand[indexB] = dem
            indexB = indexB + 1
